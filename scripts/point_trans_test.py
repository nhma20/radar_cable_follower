...
class IdDrawer(Node):
    ...
    def __init__(self):
        ...
        self.seen_ID_points = [] # [ [x,y,z,ID], [x,y,z,ID], ... ]
    ...
    def move_origin_to_lowest_ID(self, pl_list, rot_x, rot_y, z):
        
        x_tmp, y_tmp, z_tmp = 0

        for i in range(len(pl_list)):

            if pl_list[i][-1] == self.lowest_ID:
        ...
        return pl_list[index][0]-x_tmp, pl_list[index][1]-y_tmp, pl_list[index][2]-z_tmp,  
    ...
    def draw_image(self):
        ...
        # find lowest ID and initialize seen ID points list with first received powerline msg
        if self.lowest_ID == -1:
            temp_lowest_ID = 99999999
            for i in range(pl_temp.count):
                if temp_lowest_ID > self.seen_ID_points[i][-1]:
                    temp_lowest_ID = self.seen_ID_points[i][-1]
                self.seen_ID_points.append([pl_temp.poses[i].position.x, pl_temp.poses[i].position.y, pl_temp.poses[i].position.z, pl_temp.ids[i]])
            self.lowest_ID = temp_lowest_ID   
        ...
        for i in range(len(pl)):
            ...
            # search if ID already seen before
            search_ID = pl[i][-1]

            is_ID_in_list = False
            for j in range(len(self.seen_ID_points)):

                if self.seen_ID_points[j,-1] == search_ID: 
                    # update existing ID points
                    is_ID_in_list = True
                    x_mvd_orig, y_mvd_orig, z_mvd_orig = self.move_origin_to_lowest_ID(pl, rot_x, rot_y, pl[i][2])
                    self.seen_ID_points[j][0] = x_mvd_orig # x, ought to be 0
                    self.seen_ID_points[j][1] = y_mvd_orig # y
                    self.seen_ID_points[j][2] = z_mvd_orig # z
                    self.seen_ID_points[j][3] = pl[i][3] # ID
                    break

            if not is_ID_in_list:
                # append new ID point
                x_mvd_orig, y_mvd_orig, z_mvd_orig = self.move_origin_to_lowest_ID(pl, i)
                self.seen_ID_points.append([x_mvd_orig, y_mvd_orig, z_mvd_orig, pl[i][3]])
